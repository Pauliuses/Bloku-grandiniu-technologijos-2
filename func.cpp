...
string merkle_tree(vector<transac> transakcijos)
{
    ...
    //cout << copy_of_transactions[0] << endl;
...
vector<transac> generate_transactions(int size, vector<user> useriai)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        else if (transakcijos.at(i).sum <= useriai[sender_id].balance || Hashing(transakcijos.at(i).sender_public_key + transakcijos.at(i).recipient_public_key + to_string(transakcijos.at(i).sum)) == transakcijos.at(i).transaction_id_hash)
        {
...
void print_to_file(vector<block_header> blockchain_blocks, int x, int bandymai, int kelintas)
{
    ...
    //for(int i = 0; i < blockchain_blocks.at(x).transactions.size(); i++)
    //fr << blockchain_blocks.at(x).transactions.at(i).sender_public_key << " keiciasi " << blockchain_blocks.at(x).transactions.at(i).recipient_public_key <<endl;
